...
module.exports = function () {
    ...
    this.defineStep(/^I select "(.*?)" payment method$/, { timeout: 2000000 }, async function (option) {
        switch (option) {
            case "BACS":
    ...
    this.defineStep(/^the application id is displayed$/, { timeout: 2000000 }, async function () {
    ...
    this.defineStep(/^I send the application$/, { timeout: 2000000 }, async function () {
        fieldText = await this.selectPayment.sendApplicationButton();
        fieldText.click();
    });

    //     When I enter "000000000" in the card number
    this.defineStep(/^I enter "(.*?)" in the card number$/, { timeout: 2000000 }, async function (text) {
    ...
    this.defineStep(/^I enter "(.*?)" in the email on the payment screen$/, { timeout: 2000000 }, async function (text) {
    ...
    this.defineStep(/^I submit card details$/, { timeout: 2000000 }, async function () {
        fieldText = await this.selectPayment.submitCardButton();
        fieldText.click();
    });

    //When I click on 'Confirm payment' on the payment page
    this.defineStep(/^I click on Confirm payment on the payment page$/, { timeout: 2000000 }, async function () {
