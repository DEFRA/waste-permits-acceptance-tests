...
module.exports.interceptor = function() {
	...
	function XMLHttpRequestInterceptor(original) {
		...
		var xhri = function () {
			...
			this._sendRequestsToNodeProxy = false;//set to true if you wish to use interception server
			...
			this.open = function open(method, url, asynchronous) {
				...
				if (matchRoute(this.route) && asynchronous) {
					...
					if (this._sendRequestsToNodeProxy) {
						this._nodeRequest = new original();
						this._nodeRequest.open("POST", "http://localhost:5050");
						this._nodeRequestArgs = { url: url, method: method };
					}
			...
			this.convertQueryStringToJson = function (queryString) {
